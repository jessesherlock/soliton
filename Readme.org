...
soliton [sol-i-ton]; noun; a solitary wave (as in a gaseous plasma) that propagates with little loss of energy and retains its shape and speed after colliding with another such wave

Soliton is a Clojure/Clojurescript lens library that covers well behaved lenses as well as useful lens-adjacent optics.

** Install

~com.jessesherlock/soliton {:mvn/version "0.1.3"}~ / ~[com.jessesherlock/soliton "0.1.3"]~
...
(require '[soliton.async :refer [aover]])
...
(a/<! (aover [:foo :bar] get-api-result {:foo {:bar :arg}}))
; {:foo {:bar {:result 1234}}})

#+END_SRC clojure

Normal lenses and reflectors work with ~aover~, reflectors implement the Async-Over protocol used by ~aover~, there is also an ~areflect~ function to reflect over an async function directly
